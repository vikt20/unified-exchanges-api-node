...
export default class BinanceUserData extends BinanceFutures implements IUserDataManager {
    private static readonly POSITION_RISK_REFRESH_INTERVAL_MS = 1000;
    private static readonly POSITION_RISK_DEBOUNCE_MS = 100;

    private positionRiskRefreshTimers = new Map<string, ReturnType<typeof setTimeout>>();
    private positionRiskRefreshInFlight = new Set<string>();
    private positionRiskRefreshPending = new Set<string>();
    private positionRiskLastRequestAt = new Map<string, number>();
    private readonly enablePositionRiskEnrichment: boolean;
     ...
     */
    constructor(apiKey: string, apiSecret: string, enablePositionRiskEnrichment: boolean = false) {
    ...
    destroy() {
        ...
        for (const timer of this.positionRiskRefreshTimers.values()) clearTimeout(timer);
    ...
    setPosition = async (data: PositionData): Promise<void> => {
        ...
        const nextPosition: PositionData = position
            ? {
                ...data,
    ...
    private schedulePositionRiskRefresh(symbol: string): void {
        this.positionRiskRefreshPending.add(symbol);

        if (this.positionRiskRefreshInFlight.has(symbol) || this.positionRiskRefreshTimers.has(symbol)) return;

        const elapsed = Date.now() - (this.positionRiskLastRequestAt.get(symbol) ?? 0);
        const delay = Math.max(
            BinanceUserData.POSITION_RISK_DEBOUNCE_MS,
            BinanceUserData.POSITION_RISK_REFRESH_INTERVAL_MS - elapsed
        );
    ...
    private async refreshPositionRisk(symbol: string): Promise<void> {
        if (this.positionRiskRefreshInFlight.has(symbol)) return;
        ...
        try {
            ...
            if (this.positionRiskRefreshPending.has(symbol)) this.schedulePositionRiskRefresh(symbol);
        }
    }
