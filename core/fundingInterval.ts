 ...
 */
export function createFundingIntervalCallback(
    callback: (data: FundingData) => void,
    fetchInterval: boolean,
    fetcher: FundingIntervalFetcher,
    isActive: () => boolean = () => true
): (data: FundingData) => void {
    const intervals = new Map<string, number>();
    const refreshedHours = new Map<string, number>();
    ...
    return (data: FundingData) => {
        ...
        void request.then(interval => {
            if (isActive()) callback({ ...data, interval });
        });
    };
}
