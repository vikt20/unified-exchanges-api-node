...
import { toBookTickerFromTicker, toFundingFromTicker, toSpotBookTickerFromTicker, convertKrakenFuturesWsOrder, convertKrakenFuturesWsPosition, convertKrakenFuturesWsBalances } from './converters.js';
import crypto from 'crypto';
export default class KrakenStreams extends KrakenBase {
    subscriptions = [];
    ...
    parseSpotOhlc(message) {
        const msg = message;
        if (msg.channel !== 'ohlc' || !msg.data || msg.data.length === 0)
            return undefined;
    ...
    parseFuturesBook(message) {
        ...
        if (snapshot.feed === 'book_snapshot') {
            ...
            const asks = snapshot.asks.map(toTuple).filter((v) => v !== null);
            const bids = snapshot.bids.map(toTuple).filter((v) => v !== null);
            return {
        ...
        if (delta.feed === 'book') {
            if (!delta.product_id)
                return undefined;
            const bids = delta.side === 'buy' ? [[delta.price.toString(), delta.qty.toString()]] : [];
            const asks = delta.side === 'sell' ? [[delta.price.toString(), delta.qty.toString()]] : [];
            return {
    ...
    parseFuturesTicker(message) {
        const ticker = message;
        if (!ticker.feed || !ticker.product_id)
            return undefined;
        return toBookTickerFromTicker(ticker);
    }
    parseFuturesFunding(message) {
        const ticker = message;
        if (!ticker.feed || !ticker.product_id)
            return undefined;
    ...
    // --- Spot Streams ---
    spotDepthStream(symbols, callback, statusCallback, levels = 10) {
    ...
    futuresCandleStickStream(symbols, interval, callback, statusCallback) {
        ...
        const parseCandle = (rawMessage) => {
            const tradeMessage = rawMessage;
            if (tradeMessage.feed !== 'trade' || !tradeMessage.product_id)
                return undefined;
    ...
    futuresUserDataStream(callback, statusCallback) {
        ...
        const createClosedPositionUpdate = (position) => ({
            ...position,
            positionAmount: 0,
            isInPosition: false,
            unrealizedPnL: 0
                ...
                wsClient.on('message', (data) => {
                    ...
                    try {
                        ...
                        if (message.event === 'challenge' && typeof message.message === 'string') {
                            const originalChallenge = message.message;
                        ...
                        if (feed === 'balances' || feed === 'balances_snapshot') {
                            const balances = convertKrakenFuturesWsBalances(message);
    ...
    normalizeFuturesIntervalMs(interval) {
        const map = {
            '1m': 60 * 1000,
            '5m': 5 * 60 * 1000,
            '15m': 15 * 60 * 1000,
